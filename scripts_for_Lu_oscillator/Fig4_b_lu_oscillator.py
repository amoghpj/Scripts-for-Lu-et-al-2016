...
#Define System:
D=3.8*10**(-3)
#Figure 5 looks at variation in forcing parameter F
F_range=np.arange(1,6,0.1)
...
for i in tqdm(range(0,len(deltap))):
    ...
    for F in (F_range): 
        p=[4.5*D,F*D,D,1.4*D] # Notice that I've made the coefficient of D in the first term a variable. I'll be iterating over different values of K
    ...
    if deltap[i]<0:
        plt.plot(F_range,Y_VAL/timescale,label=names[i])
    else:
        plt.plot(F_range,Y_VAL/timescale,linestyle='dashed',label=names[i])
...
plt.legend()
plt.show()
#plt.savefig('/home/amogh/Documents/VTech/sem3GBCB/ModAndSim/Fig4_b_lu_oscillator.pdf')
